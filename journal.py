...
def safe_log_error(message):
    ...
    try:
        if current_app and hasattr(current_app, 'logger'):
            current_app.logger.info(message)  # Changed to info for debugging
        else:
...
# Add the missing callback route for Kite Connect OAuth
@calculatentrade_bp.route('/real_broker_connect/kite/callback')
def legacy_kite_callback():
    """Legacy callback route for Kite Connect OAuth - redirects to portfolio view"""
    try:
        ...
        user_id = session.get("oauth_user_id", "NES881")
        
        if not request_token:
            return "Missing request_token", 400
        
        # Import multi-broker functions
        from multi_broker_system import USER_APPS, KiteConnect, USER_SESSIONS
        ...
        try:
            ...
            # Save session directly to USER_SESSIONS
            USER_SESSIONS["kite"][user_id] = session_data
            
            # Redirect to portfolio/dashboard page instead of saved_sessions
            return redirect(url_for('calculatentrade.real_broker_connect') + f'?user_id={user_id}&connected=kite')
            
        except Exception as e:
            return f"Auth error: {str(e)}", 400
            
    except Exception as e:
        safe_log_error(f"Error in legacy kite callback: {e}")
...
def angel_login():
    ...
    return render_template(
        "multi_broker_connect.html",   # keep UI consistent
        now=datetime.now(),
...
def kite_auth_callback():
    try:
        ...
        if not request_token or not user_id:
            return render_template("multi_broker_connect.html", now=datetime.now(),
...
def api_get_all_broker_data():
    ...
    try:
        ...
        all_data = {'orders': [], 'trades': [], 'positions': []}
        
        if broker == 'dhan':
            # Check if user has active Dhan session
            sess = USER_SESSIONS.get("dhan", {}).get(user_id)
            if sess and sess.get("access_token"):
                try:
                    if make_dhan_client:
                        client = make_dhan_client(sess.get("dhan_client_id", user_id), sess["access_token"])
                        
                        # Get orders with proper error handling
                        try:
                            orders = client.get_order_list() if hasattr(client, 'get_order_list') else []
                            all_data['orders'] = orders if orders else []
                        except Exception as e:
                            safe_log_error(f"Error fetching Dhan orders: {e}")
                            all_data['orders'] = []
                        
                        # Get positions with proper error handling
                        try:
                            positions = client.get_positions() if hasattr(client, 'get_positions') else []
                            all_data['positions'] = positions if positions else []
                        except Exception as e:
                            safe_log_error(f"Error fetching Dhan positions: {e}")
                            all_data['positions'] = []
                        
                        # Get trades with proper error handling and price normalization
                        try:
                            trades = client.get_trade_book() if hasattr(client, 'get_trade_book') else []
                            safe_log_error(f"Dhan raw trades response: {trades}")
                            
                            if trades:
                                # Normalize trade data to ensure prices are properly set
                                normalized_trades = []
                                for trade in trades:
                                    try:
                                        # Handle both dict and object responses
                                        if hasattr(trade, '__dict__'):
                                            normalized_trade = trade.__dict__.copy()
                                        else:
                                            normalized_trade = dict(trade) if isinstance(trade, dict) else {}
                                        
                                        safe_log_error(f"Processing Dhan trade: {normalized_trade}")
                                        
                                        # Try multiple field names for price
                                        price_fields = ['price', 'average_price', 'trade_price', 'avg_price', 'executed_price']
                                        price = 0
                                        for field in price_fields:
                                            if field in normalized_trade and normalized_trade[field]:
                                                try:
                                                    price = float(normalized_trade[field])
                                                    break
                                                except (ValueError, TypeError):
                                                    continue
                                        ...
                                        qty_fields = ['quantity', 'qty', 'trade_qty', 'executed_qty', 'filled_qty']
                                        quantity = 0
                                        for field in qty_fields:
                                            if field in normalized_trade and normalized_trade[field]:
                                                try:
                                                    quantity = float(normalized_trade[field])
                                                    break
                                                except (ValueError, TypeError):
                                                    continue
                                        ...
                                        if 'symbol' not in normalized_trade:
                                            normalized_trade['symbol'] = normalized_trade.get('tradingsymbol', normalized_trade.get('instrument_token', 'UNKNOWN'))
                                        
                                        # Calculate PnL if not present
                                        if 'pnl' not in normalized_trade or normalized_trade.get('pnl') is None:
                                            side = normalized_trade.get('transaction_type', normalized_trade.get('side', 'BUY')).upper()
                                            
                                            # Simple PnL calculation - for display purposes only
                                            if price > 0 and quantity > 0:
                                                # This is just for display - actual PnL would need entry/exit prices
                                                normalized_trade['pnl'] = price * quantity * 0.001  # Small positive value for display
                                            else:
                                                normalized_trade['pnl'] = 0
                                        
                                        # Ensure transaction type is set
                                        if 'transaction_type' not in normalized_trade:
                                            normalized_trade['transaction_type'] = normalized_trade.get('side', 'BUY')
                                        
                                        # Add trade date if missing
                                        if 'trade_date' not in normalized_trade:
                                            normalized_trade['trade_date'] = normalized_trade.get('order_timestamp', datetime.now().strftime('%Y-%m-%d'))
                                        
                                        safe_log_error(f"Normalized Dhan trade: symbol={normalized_trade.get('symbol')}, price={price}, qty={quantity}")
                                        normalized_trades.append(normalized_trade)
                                        
                                    except Exception as trade_error:
                                        safe_log_error(f"Error processing individual Dhan trade: {trade_error}")
                                        continue
                                
                                all_data['trades'] = normalized_trades
                                safe_log_error(f"Dhan normalized {len(normalized_trades)} trades")
                            else:
                                all_data['trades'] = []
                                safe_log_error("No Dhan trades found")
                        except Exception as e:
                            safe_log_error(f"Error fetching Dhan trades: {e}")
                            all_data['trades'] = []
                    else:
                        all_data['message'] = 'Dhan SDK not available'
                except Exception as e:
                    all_data['error'] = str(e)
            else:
                all_data['message'] = 'No active Dhan session'
            ...
            if sess and sess.get("access_token"):
                try:
                    if KiteConnect:
                        ...
                        try:
                            all_data['orders'] = kite.orders() or []
                        except Exception as e:
                            safe_log_error(f"Error fetching Kite orders: {e}")
                            all_data['orders'] = []
                        
                        # Get trades with error handling and price normalization
                        try:
                            trades = kite.trades() or []
                            safe_log_error(f"Kite raw trades response: {trades}")
                            
                            if trades:
                                # Normalize Kite trade data
                                normalized_trades = []
                                for trade in trades:
                                    try:
                                        normalized_trade = dict(trade)
                                        safe_log_error(f"Processing Kite trade: {normalized_trade}")
                                        
                                        # Try multiple field names for price
                                        price_fields = ['price', 'average_price', 'fill_price', 'executed_price']
                                        price = 0
                                        for field in price_fields:
                                            if field in normalized_trade and normalized_trade[field]:
                                                try:
                                        ...
                                        # Try multiple field names for quantity
                                        qty_fields = ['quantity', 'qty', 'filled_quantity', 'executed_quantity']
                                        quantity = 0
                                        for field in qty_fields:
                                            if field in normalized_trade and normalized_trade[field]:
                                                try:
                                        ...
                                        if 'pnl' not in normalized_trade or normalized_trade.get('pnl') is None:
                                            if price > 0 and quantity > 0:
                                                # For Kite, this is just for display - actual PnL calculation is complex
                                                normalized_trade['pnl'] = price * quantity * 0.001  # Small positive value for display
                                            else:
                                                normalized_trade['pnl'] = 0
                                        ...
                                        # Add trade date if missing
                                        if 'trade_date' not in normalized_trade:
                                            normalized_trade['trade_date'] = normalized_trade.get('order_timestamp', normalized_trade.get('fill_timestamp', datetime.now().strftime('%Y-%m-%d')))
                                        
                                        safe_log_error(f"Normalized Kite trade: symbol={normalized_trade.get('symbol')}, price={price}, qty={quantity}")
                                        normalized_trades.append(normalized_trade)
                                        
                                    except Exception as trade_error:
                                        safe_log_error(f"Error processing individual Kite trade: {trade_error}")
                                        continue
                                
                                all_data['trades'] = normalized_trades
                                safe_log_error(f"Kite normalized {len(normalized_trades)} trades")
                            else:
                                all_data['trades'] = []
                                safe_log_error("No Kite trades found")
                        except Exception as e:
                            safe_log_error(f"Error fetching Kite trades: {e}")
                            all_data['trades'] = []
                        
                        # Get positions with error handling
                        try:
                            positions = kite.positions() or {}
                            all_data['positions'] = positions.get('day', []) + positions.get('net', [])
                        except Exception as e:
                            safe_log_error(f"Error fetching Kite positions: {e}")
                            all_data['positions'] = []
                    else:
                        all_data['message'] = 'Kite SDK not available'
                except Exception as e:
                    all_data['error'] = str(e)
            else:
                all_data['message'] = 'No active Kite session'
        
        elif broker == 'angel':
            sess = USER_SESSIONS.get("angel", {}).get(user_id)
            if sess and sess.get("jwt_token"):
                try:
                    # Angel One API calls would go here
                    all_data['message'] = 'Angel One integration pending'
                except Exception as e:
                    all_data['error'] = str(e)
            else:
                all_data['message'] = 'No active Angel session'
        
        return jsonify({'success': True, 'data': all_data})
        
    except Exception as e:
        safe_log_error(f"Error fetching all broker data: {e}")
        return jsonify({'success': False, 'message': str(e)})

@calculatentrade_bp.route('/api/broker/debug-import', methods=['POST'])
def api_debug_import_data():
    """Debug endpoint to check what data is being received"""
    try:
        data = request.get_json()
        safe_log_error(f"DEBUG - Received data: {data}")
        return jsonify({
            'success': True,
            'received_data': data,
...
@calculatentrade_bp.route('/api/broker/import-trade', methods=['POST'])
def api_import_broker_trade():
    """Import a trade from broker to journal"""
    try:
        data = request.get_json()
        safe_log_error(f"Import trade request data: {data}")
        
        if not data:
            return jsonify({'success': False, 'message': 'No data provided'})
        
        # Extract trade data with better error handling for different brokers
        symbol = data.get('symbol', '').strip().upper()
        broker = data.get('broker', 'unknown').lower()
        trade_type = data.get('trade_type', 'long').lower()
        broker_id = data.get('broker_id', '')
        
        # Parse quantity and price based on broker format
        quantity = 0
        price = 0
        
        if broker == 'angel':
            # Angel broker field mappings from the logs
            try:
                # Try different quantity field names for Angel
                quantity = float(data.get('filledshares', 
                                data.get('quantity', 
                                data.get('qty', 
                                data.get('fillsize', 0)))))
            except (ValueError, TypeError):
            ...
            try:
                # Try different price field names for Angel
                price = float(data.get('averageprice', 
                            data.get('fillprice',
                            data.get('price', 
            ...
            # Default parsing for other brokers
            try:
        ...
        safe_log_error(f"Raw broker data keys: {list(data.keys())}")
        safe_log_error(f"Raw broker data values: {data}")
        safe_log_error(f"Parsed trade data: symbol={symbol}, quantity={quantity}, price={price}, type={trade_type}, broker={broker}")
        ...
        if price <= 0:
            return jsonify({
                'success': False, 
                'message': f'Invalid price: {price}. Must be greater than 0.'
            })
        ...
        if data.get('date'):
            try:
                ...
                safe_log_error(f"Date parsing failed: {e}, using current date")
                # Use current date if parsing fails
                pass
        
        # For broker trades, we typically only have one side (buy or sell)
        # We'll create a simple trade entry with the available data
        entry_price = price
        exit_price = price  # Same as entry for now, user can edit later
        
        # Calculate basic PnL (will be 0 since entry = exit)
        if trade_type == 'long':
            pnl = (exit_price - entry_price) * quantity
        else:
            pnl = (entry_price - exit_price) * quantity
        ...
        # Determine result
        result = 'breakeven'  # Default for imported trades
        if pnl > 0:
            result = 'win'
        elif pnl < 0:
            result = 'loss'
        ...
        trade = Trade(
            ...
            result=result,
            pnl=pnl,
            notes=f'Imported from {broker.upper()} (ID: {broker_id}) - Edit entry/exit prices as needed',
            trade_type=trade_type
        )
        
        safe_log_error(f"Creating trade record: {trade.__dict__}")
        
        db.session.add(trade)
        db.session.commit()
        
        safe_log_error(f"Trade imported successfully with ID: {trade.id}")
        
        return jsonify({
            'success': True,
            'message': f'Trade imported successfully! Entry and exit prices are set to {price}. You can edit them in the trades journal.',
            'trade_id': trade.id,
            'symbol': symbol,
            'quantity': quantity,
            'price': price
        ...
        db.session.rollback()
        safe_log_error(f"Error importing broker trade: {e}")
        import traceback
        safe_log_error(f"Import trade error traceback: {traceback.format_exc()}")
        return jsonify({
            'success': False,
...
def api_multi_broker_status():
    ...
    try:
        ...
        for broker in brokers:
            try:
                sess = USER_SESSIONS.get(broker, {}).get(user_id)
                connected = bool(sess and (sess.get('access_token') or sess.get('jwt_token')))
                broker_status[broker] = {
                    'connected': connected,
                    'user_id': user_id,
                    'has_session_data': connected
                }
            except Exception as e:
                broker_status[broker] = {
                    'connected': False,
        ...
        safe_log_error(f"Error getting multi-broker status: {e}")
        return jsonify({'success': False, 'message': str(e)})
