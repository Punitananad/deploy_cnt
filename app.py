...
import traceback
# Configure logging
# PRODUCTION: Comment out debug logging
...
# PRODUCTION: Comment out template debugging
...
# PRODUCTION: Remove development fallback - must set FLASK_SECRET in production
app.secret_key = os.getenv('FLASK_SECRET') or 'dev-secret-change-this-in-production'
...
# PRODUCTION: Ensure these are False
...
# Legacy broker connection check function for backward compatibility
def check_broker_connection(broker, user_id):
    """Legacy broker connection check - returns empty for compatibility"""
    return {'connected': False}
...
def create_flow(state=None):
    ...
    import os
    # PRODUCTION: Comment out insecure transport
...
# PRODUCTION: Account deletion disabled - redirect to support
...
def save_universal_result(calc_type):
    ...
    try:
        ...
        elif calc_type in ['mtf', 'intraday']:
            trade_data["leverage"] = float(data.get("leverage", config['leverage']))
...
def mtf_detail(trade_id):
    ...
    trade_data = {
        ...
        'status': getattr(trade, 'status', 'open'),
        'timestamp': trade.timestamp,
        'leverage': getattr(trade, 'leverage', 4.0),
        'ltp': trade.avg_price
    }
...
def save_mtf_position_update():
    try:
        ...
        # Update leverage if provided, otherwise use existing or default
        leverage = float(data.get('leverage', getattr(trade, 'leverage', 4.0)))
...
def legacy_kite_callback():
    """Handle legacy Kite callback URL and redirect to new multi-broker system"""
    ...
    redirect_url = f'/api/multi_broker/kite/callback?request_token={request_token}'
    
    # Add state if available
    if state:
        redirect_url += f'&state={state}'
    
    # If no state, try to get user_id from session or use default
    elif not state and hasattr(session, 'get'):
...
def multi_broker_connect_page():
    ...
    for broker in ['kite', 'dhan', 'angel']:
        ...
        if status['connected'] or multi_status['connected']:
            ...
            connection_key = f"{broker}_{user_id}"
    ...
    try:
        ...
        print(f"Error rendering multi_broker_connect template: {e}")
        # Fallback response
        return f"<h1>Multi Broker Connect</h1><p>Successfully connected! Login success: {login_success}, User ID: {user_id}</p><a href='/'>Go to Home</a>"
...
def get_all_trading_data():
    """Simple endpoint that always returns success to prevent red prompt"""
    broker = request.args.get('broker', 'unknown')
    user_id = request.args.get('user_id', current_user.email if current_user else 'unknown')
    
    # Check if token is valid for Dhan
    if broker == 'dhan':
        from token_store import is_token_valid
        if not is_token_valid():
            return jsonify({
                ...
                'broker': broker,
                'user_id': user_id,
                'error': 'Dhan API token expired. Please update token.',
                'message': 'Token authentication required'
            }), 401
    
    # Always return success to prevent red prompt in frontend
    return jsonify({
        'success': True,
        'connected': False,
        'broker': broker,
        ...
        'message': f'{broker.upper()} registration successful'
    }), 200

# Real broker connect route (the one causing 404 in frontend)
@app.route('/calculatentrade_journal/real_broker_connect', methods=['GET'])
def real_broker_connect():
    """Handle real broker connect requests from frontend"""
    user_id = request.args.get('user_id', 'P54947093')
    connected = request.args.get('connected', 'unknown')
    
    # Redirect to the multi-broker connect page
    return redirect(f'/multi_broker_connect?user_id={user_id}')

# Catch-all route for multi_broker_connect variations
@app.route('/multi_broker_connect/<path:subpath>')
def multi_broker_connect_catchall(subpath):
    """Handle any multi_broker_connect URL variations"""
    return redirect(url_for('multi_broker_connect', **request.args))

# Angel One login now handled by multi-broker system
# Redirect routes for backward compatibility
@app.route("/angel/login")
def angel_login():
    """Redirect to multi-broker Angel login"""
    user_id = request.args.get("user_id", "P54947093")
    return redirect(f"/api/multi_broker/angel/login?user_id={user_id}")

@app.route("/angel/login/password", methods=["POST"])
...
#     print("Legacy multi-broker system integrated successfully")
# except ImportError as e:
#     print(f"Legacy multi-broker system not available: {e}")
# except Exception as e:
#     print(f"Error integrating legacy multi-broker system: {e}")
